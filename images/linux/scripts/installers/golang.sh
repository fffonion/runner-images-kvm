...
for TOOLSET_VERSION in ${TOOLSET_VERSIONS[@]}; do
    PACKAGE_TAR_NAME=$(echo "$PACKAGE_TAR_NAMES" | grep "^go${TOOLSET_VERSION}.${PLATFORM_NAME}-${ARCH}.tar.gz$" | sort -V | tail -1)
    ...
    setEtcEnvironmentVariable "GOROOT_$(echo $GOLANG_VERSION| cut -d'.' -f1)_$(echo $GOLANG_VERSION| cut -d'.' -f2)_${arch^^}" $GOLANG_VERSION_PATH
done
